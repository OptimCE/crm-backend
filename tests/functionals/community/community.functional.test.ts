...
describe("(Functional) Community Module", () => {
  ...
  describe("(Functional) Get Users", () => {
    it.each(testCasesGetUsers)("GET /communities/users : $description", async ({ query, orgs, status_code, expected_error_code, check_data }) => {
      const appModule = await import("../../../src/app.js");
      const app = appModule.default;
  ...
  describe("(Functional) Get Admins", () => {
    it.each(testCasesGetAdmins)("GET /communities/admins : $description", async ({ query, orgs, status_code, expected_error_code, check_data }) => {
      const appModule = await import("../../../src/app.js");
      const app = appModule.default;
  ...
  describe("(Functional) Create Community", () => {
    it.each(testCasesCreateCommunity)(
      ...
      async ({ body, orgs, status_code, expected_error_code, expected_data, external_mocks }) => {
        ...
        const response = await request(app).post("/communities/").send(body).set("x-user-id", "auth0|member").set("x-user-orgs", orgs);
  ...
  describe("(Functional) Leave Community", () => {
    it.each(testCasesLeave)(
      ...
      async ({ target_comm_id, orgs, status_code, expected_error_code, expected_data, external_mocks }) => {
        ...
        const response = await request(app).delete(`/communities/leave/${target_comm_id}`).set("x-user-id", "auth0|admin").set("x-user-orgs", orgs);
