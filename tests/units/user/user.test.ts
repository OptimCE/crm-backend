...
describe("(Unit) User Module", () => {
  ...
  describe("(Unit) Get Profile", () => {
    it.each(testCasesGetProfile)("GET /users/ : $description", async ({ id_user, status_code, expected_error_code, expected_data, mocks, orgs }) => {
  ...
  describe("(Unit) Update Profile", () => {
    it.each(testCasesUpdateProfile)("PUT /users/ : $description", async ({ body, status_code, expected_error_code, expected_data, mocks, orgs }) => {
      if (mocks?.userRepo) await mockUserRepositoryModule(mocks.userRepo);
