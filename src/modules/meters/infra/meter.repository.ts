...
import {add, applyFilters, FilterDef} from "../../../shared/database/filters.js";
...
export class MeterRepository implements IMeterRepository {
    ...
    async addMeterConsumptions(id_sharing: number, consumptions: (Partial<MeterConsumption> & { ean: string })[], query_runner?: QueryRunner): Promise<void> {
    ...
    async getLastMeterData(ean: string, query_runner?: QueryRunner): Promise<MeterData | null> {
        ...
        return manager.findOne(MeterData, {
            where: { meter: { EAN: ean } as any, community: { id: internal_community_id } },
            order: { start_date: "DESC" },
    ...
    getMeter(id: string, query_runner?: QueryRunner): Promise<Meter|null> {
        ...
        qb = qb.where("meter.EAN = :ean", { ean: id })
    ...
    async getMeterConsumptions(ean: string, query: MeterConsumptionQuery, query_runner?: QueryRunner): Promise<MeterConsumption[]> {
        ...
        qb = qb.where("consumption.meter = :ean", { ean })
            .andWhere("consumption.community = :commId", { commId: internal_community_id });
