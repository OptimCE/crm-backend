...
import {
    CreateMeterDTO,
    MeterConsumptionDTO,
    MeterConsumptionQuery, MeterPartialQuery,
    MetersDTO, PartialMeterDTO, PatchMeterDataDTO,
    UpdateMeterDTO
...
export class MeterService implements IMeterService {
    constructor(@inject("MeterRepository") private meterRepository: IMeterRepository,
    ...
    async addMeter(new_meter: CreateMeterDTO, query_runner?: QueryRunner): Promise<void> {
        ...
        try {
            ...
            await this.meterRepository.addMeterData(new_meter.EAN, {
                ...
                member: initial.member_id ? { id: initial.member_id } as any : null,
                sharing_operation: initial.sharing_operation_id ? { id: initial.sharing_operation_id } as any : null
            }, query_runner);
    ...
    async patchMeterData(patched_meter_data: PatchMeterDataDTO, query_runner?: QueryRunner): Promise<void> {
        ...
        try {
            await this.meterRepository.addMeterData(patched_meter_data.EAN, {
                ...
                member: patched_meter_data.member_id ? { id: patched_meter_data.member_id } as any : null,
                sharing_operation: patched_meter_data.sharing_operation_id ? { id: patched_meter_data.sharing_operation_id } as any : null
            }, query_runner);
        } catch (err: any) {
            if (err instanceof AppError || err.constructor.name === 'AppError') {
                throw err;
