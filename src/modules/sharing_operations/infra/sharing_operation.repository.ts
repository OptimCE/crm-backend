...
export class SharingOperationRepository implements ISharingOperationRepository {
    ...
    async getSharingOperationById(id_sharing: number, query_runner?: QueryRunner): Promise<SharingOperation | null> {
        ...
        qb = qb.where("sharing_op.id = :id", { id: id_sharing })
    ...
    async getSharingOperationConsumption(id_sharing: number, query: SharingOperationConsumptionQuery, query_runner?: QueryRunner): Promise<SharingOpConsumption[] | null> {
        const manager = query_runner ? query_runner.manager : this.dataSource.manager;
    ...
    async getAuthorizedEans(id_sharing: number, query_runner?: QueryRunner): Promise<Set<string>> {
        ...
        // We query MeterData because it holds the relationship between Meter and SharingOperation
        // We select DISTINCT EANs to get a unique list of meters ever associated with this operation.
        const result = await manager.createQueryBuilder(MeterData, "meter_data")
            .select("meter_data.ean") // Note: Assumes the JoinColumn matches the property 'meter' but mapped to 'ean' in selection if 'meter' is the relation.
            // Better to select the relation id. Since MeterData has @ManyToOne meter @JoinColumn({name:'ean'}), we can select "meter_data.meter".
            .leftJoin("meter_data.meter", "meter")
            .where("meter_data.sharing_operation = :id", { id: id_sharing })
            .getRawMany(); // returns [{ meter_EAN: '...' }] or similar based on driver

        // Map results to a Set of strings.
        // Note: raw result keys depend on naming strategy. Usually 'meter_data_ean' or 'meter_EAN'.
        // To be safe, let's select just the column.

        // Revised query for safety:
        const eans = await manager.createQueryBuilder(MeterData, "meter_data")
    ...
    async addSharingKeyEntry(id_sharing: number, id_key: number, start_date: Date, status: SharingKeyStatus, query_runner?: QueryRunner): Promise<SharingOperationKey> {
        const manager = query_runner ? query_runner.manager : this.dataSource.manager;
        const internal_community_id = await this.authContext.getInternalCommunityId(query_runner);

        const entity = manager.create(SharingOperationKey, {
            sharing_operation: { id: id_sharing } as any,
            allocation_key: { id: id_key } as any,
            community: { id: internal_community_id } as any,
    ...
    async closeSpecificKeyEntry(id_sharing: number, id_key: number, end_date: Date, query_runner?: QueryRunner): Promise<void> {
        ...
        await manager.createQueryBuilder(SharingOperationKey, "key")
