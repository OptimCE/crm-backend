...
export class SharingOperationService implements ISharingOperationService {
    ...
    constructor(@inject("SharingOperationRepository") private sharing_operationRepository: ISharingOperationRepository,
    ...
    async addConsumptionDataToSharing(dto: AddConsumptionDataDTO, query_runner?: QueryRunner): Promise<void> {
        ...
        const meterMap = new Map<string, Map<string, {
            timestamp: Date;
            gross: number;
            net: number;
            shared: number;
        ...
        for (const sheetName of sheetNames) {
            ...
            const columnMap = headers
                .map((header, index) => {
                    ...
                    return type && ean && authorizedEansSet.has(ean)
                        ? { index, ean: ean, type }
                        : null;
                })
    ...
    async addMeterToSharing(new_meters_to_operation: AddMeterToSharingOperationDTO, query_runner?: QueryRunner): Promise<void> {
        ...
        try {
            for (const ean of ean_list) {
                await this.meterRepository.addMeterData(ean, {
     ...
     */
    async downloadSharingOperationConsumptions(id_sharing: number, query: SharingOperationConsumptionQuery, query_runner?: QueryRunner): Promise<Buffer> {
    ...
    async patchMeterStatus(patched_meter_status: PatchMeterToSharingOperationDTO, query_runner?: QueryRunner): Promise<void> {
        ...
        try {
            await this.meterRepository.addMeterData(id_meter, {
                sharing_operation: { id: id_sharing } as any,
