...
export class KeyService implements IKeyService {
    constructor(@inject("KeyRepository") private keyRepository: IKeyRepository,
     ...
     */
    async downloadKey(key_id: number): Promise<any> {
        try {
            ...
            worksheetHome.columns.forEach((column: any) => {
                const length = column.header.length;
                const maxWidth = column.values.map((v: any) => v.toString().length);
                const minWidth = 15;
     ...
     */
    async addCreditToExcel(workbook: any): Promise<any> {
        try {
